...
const defaultDeps = { spawnSync: spawnSync, readFileSync: (path, encoding) => readFileSync(path, encoding), writeFileSync, mkdirSync, existsSync, promptUser: readlinePrompt };
export function resolveBinaryPath(deps = defaultDeps) {
...
export function buildLaunchdPlist(binaryPath, logPath, nodeBinDir) {
...
export function buildSystemdUnit(binaryPath) {
    return `[Unit]
...
export function setupDaemonService(deps = defaultDeps) {
...
function parseCipherYml(content) {
    try {
        ...
        for (const line of content.split("\n")) {
            ...
            if (inLlmSection && /^[^\s]/.test(line))
                break; // left llm section
            if (inLlmSection) {
...
async function pickSummarizer(deps, cipherConfigPath) {
...
export function parseCipherConfig(cipherYmlPath, deps) {
    try {
        ...
        if (llmSection) {
            ...
            if (urlMatch)
                llmBaseURL = urlMatch[1];
            const providerMatch = llmSection[0].match(/provider:\s*(\S+)/);
            if (providerMatch && providerMatch[1] === "ollama")
                backend = "ollama";
        }
        // Detect remote
...
export function installCipherPackage(deps) {
...
export function installCipherWrapper(deps) {
...
export function mergeCipherSettings(existing, config) {
    const settings = JSON.parse(JSON.stringify(existing));
    settings.mcpServers = settings.mcpServers ?? {};
...
export function installClaudeServer(deps, config) {
...
export async function install(deps = defaultDeps) {
    ...
    if (deps.existsSync(settingsPath)) {
        try {
            existing = JSON.parse(deps.readFileSync(settingsPath, "utf-8"));
        }
        catch { }
    }
    ...
    if (!qdrantOk)
        console.warn("Warning: Qdrant not responding — run: lossless-claude doctor");
