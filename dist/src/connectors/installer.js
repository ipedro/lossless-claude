...
function installMarkdown(content, filePath, writeMode) {
...
function installMcpJson(filePath) {
    mkdirSync(dirname(filePath), { recursive: true });
    let existing = {};
...
function removeMcpJson(filePath) {
    if (filePath.endsWith('.toml'))
        return false; // TOML removal not supported
    if (!existsSync(filePath))
        return false;
    let config;
    try {
...
export function installConnector(agentIdOrName, type, cwd = process.cwd()) {
    const agent = findAgent(agentIdOrName);
    if (!agent)
        throw new Error(`Unknown agent: ${agentIdOrName}`);
    ...
    if (!configPath)
        throw new Error(`No config path defined for ${agent.name} with type ${connectorType}`);
...
export function removeConnector(agentIdOrName, type, cwd = process.cwd()) {
    const agent = findAgent(agentIdOrName);
    if (!agent)
        throw new Error(`Unknown agent: ${agentIdOrName}`);
    const connectorType = type ?? agent.defaultType;
...
export function listConnectors(cwd = process.cwd()) {
    const installed = [];
