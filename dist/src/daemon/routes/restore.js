...
function readClaudeMdFiles(cwd) {
    const paths = [
                ...
                try {
                    ...
                    if (claudeMdContent) {
                        ...
                        if (!existing || existing.content_hash !== hash) {
                            db.prepare(`INSERT INTO session_instructions (id, content, content_hash, updated_at)
                 VALUES (1, ?, ?, datetime('now'))
                 ON CONFLICT(id) DO UPDATE SET
                   content = excluded.content,
                   content_hash = excluded.content_hash,
                   updated_at = excluded.updated_at`).run(claudeMdContent, hash);
                        }
                    }
                }
                catch { /* non-fatal */ }
