...
export async function handleSensitive(argv, cwd, configPath) {
    ...
    switch (sub) {
        ...
        default: {
            return {
                exitCode: 1,
                stdout: "Usage: lcm sensitive <list|add|remove|test|purge> [options]\n",
            };
        }
    }
}
async function sensitiveList(cwd, configPath) {
    let globalUserPatterns = [];
    try {
    ...
    lines.push("");
    lines.push(`Project patterns (${patternsFile}):`);
...
async function sensitiveAdd(args, cwd, configPath) {
...
async function sensitiveRemove(args, cwd) {
    const pattern = args.find((a) => !a.startsWith("--"));
    if (!pattern) {
        return {
            exitCode: 1,
...
async function sensitiveTest(args, cwd, configPath) {
    ...
    const projectPatterns = await ScrubEngine.loadProjectPatterns(patternsFile);
    const allPatterns = [
        ...BUILT_IN_PATTERNS.map((p) => ({ source: p, kind: "built-in" })),
        ...globalUserPatterns.map((p) => ({ source: p, kind: "global" })),
        ...projectPatterns.map((p) => ({ source: p, kind: "project" })),
    ];
    const matched = [];
...
async function sensitivePurge(args, cwd) {
    const hasYes = args.includes("--yes");
    const purgeAll = args.includes("--all");
    if (!hasYes) {
        return {
            exitCode: 1,
            stdout: "Error: lcm sensitive purge requires --yes to confirm.\n" +
