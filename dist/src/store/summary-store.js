...
export class SummaryStore {
    ...
    async insertSummary(input) {
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
            .run(input.summaryId, input.conversationId, input.kind, depth, input.content, input.tokenCount, fileIds, earliestAt, latestAt, descendantCount, descendantTokenCount, sourceMessageTokenCount);
        const row = this.db
    ...
    async replaceContextRangeWithSummary(input) {
        ...
        try {
         ...
         ORDER BY ordinal`)
                .all(conversationId);
            const updateStmt = this.db.prepare(`UPDATE context_items
         SET ordinal = ?
    ...
    async insertLargeFile(input) {
        this.db
            .prepare(`INSERT INTO large_files (file_id, conversation_id, file_name, mime_type, byte_size, storage_uri, exploration_summary)
       VALUES (?, ?, ?, ?, ?, ?, ?)`)
            .run(input.fileId, input.conversationId, input.fileName ?? null, input.mimeType ?? null, input.byteSize ?? null, input.storageUri, input.explorationSummary ?? null);
        const row = this.db
            .prepare(`SELECT file_id, conversation_id, file_name, mime_type, byte_size, storage_uri, exploration_summary, created_at
