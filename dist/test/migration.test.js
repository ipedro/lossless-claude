...
describe("runLcmMigrations summary depth backfill", () => {
    ...
    `);
        db.prepare(`INSERT INTO conversations (conversation_id, session_id) VALUES (?, ?)`).run(1, "legacy-session");
        const insertSummaryStmt = db.prepare(`INSERT INTO summaries (summary_id, conversation_id, kind, content, token_count, file_ids)
       VALUES (?, ?, ?, ?, ?, '[]')`);
        ...
        const insertMessageStmt = db.prepare(`INSERT INTO messages (message_id, conversation_id, seq, role, content, token_count, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?)`);
        ...
        const linkMessageStmt = db.prepare(`INSERT INTO summary_messages (summary_id, message_id, ordinal)
       VALUES (?, ?, ?)`);
        linkMessageStmt.run("sum_leaf_a", 1, 0);
        linkMessageStmt.run("sum_leaf_a", 2, 1);
        linkMessageStmt.run("sum_leaf_b", 3, 0);
        const linkStmt = db.prepare(`INSERT INTO summary_parents (summary_id, parent_summary_id, ordinal)
       VALUES (?, ?, ?)`);
        ...
        const depthRows = db
            .prepare(`SELECT summary_id, depth, earliest_at, latest_at, descendant_count,
        ...
        const descendantCountBySummaryId = new Map(depthRows.map((row) => [row.summary_id, row.descendant_count]));
        const descendantTokenCountBySummaryId = new Map(depthRows.map((row) => [row.summary_id, row.descendant_token_count]));
        const sourceMessageTokenCountBySummaryId = new Map(depthRows.map((row) => [row.summary_id, row.source_message_token_count]));
        ...
        expect(new Date(leafAEarliest).getTime()).toBeLessThanOrEqual(new Date(leafALatest).getTime());
        expect(new Date(leafBEarliest).getTime()).toBeLessThanOrEqual(new Date(leafBLatest).getTime());
        expect(new Date(condensed1Earliest).getTime()).toBeLessThanOrEqual(new Date(condensed1Latest).getTime());
        expect(new Date(condensed2Earliest).getTime()).toBeLessThanOrEqual(new Date(condensed2Latest).getTime());
        expect(new Date(condensed1Earliest).getTime()).toBeLessThanOrEqual(new Date(leafAEarliest).getTime());
        expect(new Date(condensed1Latest).getTime()).toBeGreaterThanOrEqual(new Date(leafBLatest).getTime());
